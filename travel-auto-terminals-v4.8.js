(() => {
  ...
  async function createTerminal({tripId,userId,cityId,name,address,hubType,arrivalAt,departureAt,buffer,confirmed,key,notes}){
    const order=await nextHubOrder(tripId);
    const {data,error}=await db.from('travel_hubs').insert({trip_id:tripId,city_id:cityId||null,user_id:userId,hub_key:key,name,hub_type:hubType,address:address||null,planned_arrival_at:arrivalAt||null,planned_departure_at:departureAt||null,buffer_minutes:buffer||0,is_confirmed:!!confirmed,notes:notes||null,order_index:order}).select().single();if(error)throw error;return data;
  }
  ...
  async function handleAutoSubmit(event,form,mode,meta){
    ...
    try{
      ...
      const depCity=await ensureCity(tripId,user.id,fromName,fromAddress),arrCity=await ensureCity(tripId,user.id,toName,toAddress);
      const terminalArrival=new Date(new Date(dep).getTime()-meta.buffer*60000).toISOString(),stamp=Date.now();
      created.departureHub=await createTerminal({tripId,userId:user.id,cityId:depCity?.id,name:fromName,address:fromAddress,hubType:meta.hubType,arrivalAt:terminalArrival,departureAt:dep,buffer:meta.buffer,confirmed:$('#bConfirmed',form).value==='true',key:`auto-${mode}-departure-${stamp}`,notes:`Automatically created departure ${meta.hubType.replaceAll('_',' ')} for this ${mode}.`});
      created.arrivalHub=await createTerminal({tripId,userId:user.id,cityId:arrCity?.id,name:toName,address:toAddress,hubType:meta.hubType,arrivalAt:arr,departureAt:null,buffer:0,confirmed:$('#bConfirmed',form).value==='true',key:`auto-${mode}-arrival-${stamp}`,notes:`Automatically created arrival ${meta.hubType.replaceAll('_',' ')} for this ${mode}.`});
      const depLocal=localParts(terminalArrival,$('#bDepZone',form).value),seq=await nextSequence(tripId,depCity?.id||null,depLocal.date);
      const {data:route,error:re}=await db.from('travel_route_stops').insert({trip_id:tripId,city_id:depCity?.id||null,user_id:user.id,route_date:depLocal.date,sequence:seq,stop_type:'hub',hub_id:created.departureHub.id,title:fromName,planned_start:depLocal.time,planned_end:$('#bDepTime',form).value,travel_mode:'other',travel_minutes:0,status:'planned',notes:`Be at ${fromName} about ${meta.buffer} minutes before departure.`,photo_query:fromName}).select().single();if(re)throw re;created.route=route;
      const {data:last}=await db.from('travel_transport_segments').select('order_index').eq('trip_id',tripId).order('order_index',{ascending:false}).limit(1);
      const payload={trip_id:tripId,user_id:user.id,segment_key:`custom-${stamp}`,order_index:(last?.[0]?.order_index||0)+1,mode,provider:$('#bProvider',form).value.trim()||null,service_number:$('#bService',form).value.trim()||null,from_name:fromName,from_address:fromAddress,to_name:toName,to_address:toAddress,depart_at:dep,arrive_at:arr,duration_minutes:$('#bDuration',form).value?Number($('#bDuration',form).value):null,transport_detail:$('#bDetails',form).value.trim()||null,is_confirmed:$('#bConfirmed',form).value==='true'};
      const {data:segment,error:se}=await db.from('travel_transport_segments').insert(payload).select().single();if(se)throw se;created.segment=segment;
      await upload($('#bPhoto',form).files?.[0],user.id,tripId,segment.id);
      ...
      try{if(created.segment)await db.from('travel_transport_segments').delete().eq('id',created.segment.id);if(created.route)await db.from('travel_route_stops').delete().eq('id',created.route.id);if(created.departureHub)await db.from('travel_hubs').delete().eq('id',created.departureHub.id);if(created.arrivalHub)await db.from('travel_hubs').delete().eq('id',created.arrivalHub.id)}catch{}
